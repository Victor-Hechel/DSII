...
def self.validaTelefone(tel)
	tel.strip
	loop do
		if !((tel.match(/\([0-9]{2}\)9[0-9]{8}/) && tel.length == 13) || (tel.match(/[0-9]{2}9[0-9]{8}/) && tel.length == 11) || 
			(tel.match(/\([0-9]{2}\)9[0-9]{4}-[0-9]{4}/) && tel.length == 14))
			puts("Telefone invalido! Digite novamente: ")
		elsif ContatoDAO.new.existeTel(tel) == true
